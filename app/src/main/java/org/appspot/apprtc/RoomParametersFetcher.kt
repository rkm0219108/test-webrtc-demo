...
class RoomParametersFetcher(
    ...
    private fun roomHttpResponseParse(response: String) {
        ...
        try {
            ...
            val json = roomJson.getString("params")
            ...
            if (!initiator) {
                ...
                for (i in 0 until messages.length()) {
                    ...
                    when (messageType) {
                        "offer" -> offerSdp = SessionDescription(
    ...
    private fun requestTurnServers(url: String): List<PeerConnection.IceServer> {
        val turnServers = ArrayList<PeerConnection.IceServer>()
    ...
    private fun iceServersFromPCConfigJSON(pcConfig: String): MutableList<PeerConnection.IceServer> {
        val json = JSONObject(pcConfig)
        val servers = json.getJSONArray("iceServers")
        val ret = ArrayList<PeerConnection.IceServer>()
        for (i in 0 until servers.length()) {
            val server = servers.getJSONObject(i)
            val url = server.getString("urls")
            val credential = if (server.has("credential")) server.getString("credential") else ""
            val turnServer = PeerConnection.IceServer.builder(url)
                    .setPassword(credential)
                    .createIceServer()
            ret.add(turnServer)
        }
