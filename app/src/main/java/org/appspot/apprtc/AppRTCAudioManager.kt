 ...
 */
class AppRTCAudioManager private constructor(private val context: Context) {
    ...
    private fun onProximitySensorChangedState() {
        ...
        if (audioDevices.size == 2 && audioDevices.contains(AudioDevice.EARPIECE)
            ...
            when {
                proximitySensor == null -> setAudioDeviceInternal(AudioDevice.SPEAKER_PHONE)
                proximitySensor!!.sensorReportsNearState() -> // Sensor reports that a "handset is being held up to a person's ear",
    ...
    init {
        ...
        proximitySensor = AppRTCProximitySensor.create(context, Runnable
    ...
    fun updateAudioDeviceState() {
        ...
        val needBluetoothAudioStart = bluetoothManager.state == AppRTCBluetoothManager.State.HEADSET_AVAILABLE && (userSelectedAudioDevice == AudioDevice.NONE || userSelectedAudioDevice == AudioDevice.BLUETOOTH)

        // Need to stop Bluetooth audio if user selected different device and
        // Bluetooth SCO connection is established or in the process.
        val needBluetoothAudioStop = (bluetoothManager.state == AppRTCBluetoothManager.State.SCO_CONNECTED || bluetoothManager.state == AppRTCBluetoothManager.State.SCO_CONNECTING) && userSelectedAudioDevice != AudioDevice.NONE && userSelectedAudioDevice != AudioDevice.BLUETOOTH
        ...
        val newAudioDevice = when {
            bluetoothManager.state == AppRTCBluetoothManager.State.SCO_CONNECTED -> // If a Bluetooth is connected, then it should be used as output audio
                // device. Note that it is not sufficient that a headset is available;
                // an active SCO channel must also be up and running.
                AudioDevice.BLUETOOTH
            hasWiredHeadset -> // If a wired headset is connected, but Bluetooth is not, then wired headset is used as
                // audio device.
...
//        private const val SPEAKERPHONE_TRUE = "true"
        ...
        /** Construction.  */
        internal fun create(context: Context): AppRTCAudioManager {
