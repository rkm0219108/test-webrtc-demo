 ...
 */
class CaptureQualityController(private val captureFormatText: TextView, private val callEvents: CallFragment.OnCallEvents) : SeekBar.OnSeekBarChangeListener {
    ...
    private var targetBandwidth: Double = 0.toDouble()
    ...
    override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
        ...
        var maxCaptureBandwidth = java.lang.Long.MIN_VALUE
        for (format in formats) {
            maxCaptureBandwidth = Math.max(maxCaptureBandwidth, format.width.toLong() * format.height.toLong() * format.framerate.max.toLong())
        }
    ...
    private fun calculateFramerate(bandwidth: Double, format: CaptureFormat): Int {
        return Math.round(
                Math.min(format.framerate.max, Math.round(bandwidth / (format.width * format.height)).toInt()) / 1000.0).toInt()
    }
